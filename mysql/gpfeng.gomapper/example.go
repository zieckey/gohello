...
func main() {
	...
	res, err := mapper.Insert("insertStmt", "Lilei", "LL", "zzz@163.com", true)
	if err != nil {
	...
	res, err = mapper.Insert("insertStmt", Record{FirstName: "David", LastName: "YY", Email:"xxx@163.com", EmailVerified: false})
	if err != nil {
		fmt.Printf("Insert failed: %s\n", err.Error())
		return
	}
	...
	for rows.Next() {
		var rec Record
		err = rows.Scan(&rec.Id, &rec.FirstName, &rec.LastName, &rec.Email, &rec.EmailVerified, &rec.CreatedAt)
		if err != nil {
	...
	res, err = mapper.Update("updateById", Record{Id: 4, FirstName: "Lilei", LastName: "LL"})
	if err != nil {
		fmt.Printf("GoMapper Update failed: %s\n", err.Error())
		return
	}
