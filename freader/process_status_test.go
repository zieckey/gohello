...
func TestSscanfAndTime(t *testing.T) {
    ...
    st, _ := time.Parse("2006/01/02-15:04:05.9999", start)
    ...
    s := st.Format("2006/01/02-15:04:05.9999")
    assert.Equal(t, s, start)
}

func TestNewProcessStatus(t *testing.T) {
    t1, err := time.Parse("2006/01/02-15:04:05.9999", "2015/08/28-23:48:34.7161")
    t2, err := time.Parse("2006/01/02-15:04:05.9999", "2015/08/28-23:48:36.7161")
    f1 := "1.txt"
    f2 := "2.txt"
    statusFile := "e:/1/.status." + strconv.FormatInt(time.Now().UnixNano(), 10)
