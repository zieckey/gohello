...
func stringEncoder(e *encodeState, v reflect.Value, quoted bool) {
	...
	if quoted {
		...
		log.Println("stringEncoder, write string : [" + v.String() + "]")
		e.string(v.String())
	}
}
...
func (e *encodeState) string(s string) (int, error) {
	//TODO UNICODE相关代码必须修改这个函数的实现
	log.Println("encodeState.string(" + s + ")")
	len0 := e.Len()
	e.WriteByte('"')
	start := 0
	for i := 0; i < len(s); {
		if b := s[i]; b < utf8.RuneSelf {
			if 0x20 <= b && b != '\\' && b != '"' && b != '<' && b != '>' && b != '&' {
				i++
				log.Println("ascii i",i,"start",start)
				continue
			}
			if start < i {
		...
		log.Println("utf8.DecodeRuneInString return c",c,"size",size)
