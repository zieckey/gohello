...
func TestHMAC() {
	for i, tt := range hmacTests {
		h := hmac.New(tt.hash, tt.key)
		if s := h.Size(); s != tt.size {
			fmt.Printf("Size: got %v, want %v", s, tt.size)
		}
		if b := h.BlockSize(); b != tt.blocksize {
			fmt.Printf("BlockSize: got %v, want %v", b, tt.blocksize)
		}
		for j := 0; j < 2; j++ {
			n, err := h.Write(tt.in)
			if n != len(tt.in) || err != nil {
				fmt.Printf("test %d.%d: Write(%d) = %d, %v", i, j, len(tt.in), n, err)
				continue
			}
			...
			for k := 0; k < 2; k++ {
				...
				if sum != tt.out {
					fmt.Printf("test %d.%d.%d: have %s want %s\n", i, j, k, sum, tt.out)
				} else {
					fmt.Printf("test passed. sum=%v\n", sum)
				}
				mysum := fmt.Sprintf("%x", myhmac(tt.key, tt.in, tt.hash))
				if mysum != tt.out {
					fmt.Printf("myhmac test %d.%d.%d: have %s want %s\n", i, j, k, mysum, tt.out)
				} else {
					fmt.Printf("test passed. myhmac sum=%v\n", sum)
				}
			}
...
*/	
	...
	mh.Write(kipad)
	hex := fmt.Sprintf("%x",mh.Sum(data))
	mh.Reset()
	
	fmt.Printf("blocksize=%v inner hex=[%v]\n", b, hex)
	
	mh.Write(kopad)
	return mh.Sum([]byte(hex))
} 


func md5hmac(key, data []byte) []byte {
/* take it from php doc :
function hmac ($key, $data)
 {
     // RFC 2104 HMAC implementation for php.
...
func TestMD5HMAC() {
	...
	key := []byte("Jefe")
 	...
 	h := md5hmac(key, data)
 	fmt.Printf("======> TestMD5HMAC len(hmac)=%v hmac=[%x]\n", len(h), h)
}
